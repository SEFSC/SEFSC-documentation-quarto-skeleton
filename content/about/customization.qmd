...
Quarto projects are configured using a `_quarto.yml` file; see the [docs](https://quarto.org/docs/guide/){target="_blank"} for more info. The new template repo contains this file already. The [metadata sharing](https://quarto.org/docs/projects/quarto-projects.html#shared-metadata){target="_blank"} feature of Quarto allows that `_quarto.yml` file up to be split into multiple files. Quarto will combine them together again when the project is rendered. We will take advantage of this feature here.
...
3. Back in the theme repo `_website.yml` file:
   ...
   ::: {.callout-note}
   Setting `contents: auto` allows the sidebar to be automatically generated, eliminating the need for manual construction or configuration. Users should therefore follow the instructions in the [docs](https://quarto.org/docs/websites/website-navigation.html#auto-generation){target="_blank"} for facilitating this feature.
   :::

4. In the theme repo `theme.scss` and `theme-dark.scss` files, assign NOAA-specific colors to variable names for easy use later. These colors were taken from the NOAA Fisheries Brand Standards and include NOAA primary colors, grays, and secondary colors for each region. For example:
